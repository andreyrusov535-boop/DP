...
async function up(db) {
  ...
  await db.exec(`
    INSERT INTO nomenclature (type, code, name, description, sort_order, metadata) VALUES
    ('receipt_form', 'written', 'Written Application', 'Traditional written request form', 1, '{"requires_signature": true, "processing_days": 3}'),
    ('receipt_form', 'electronic', 'Electronic Portal', 'Online submission through citizen portal', 2, '{"requires_signature": false, "processing_days": 1}'),
    ('receipt_form', 'phone', 'Phone Call', 'Request received via phone call', 3, '{"requires_signature": false, "processing_days": 1}'),
    ('receipt_form', 'email', 'Email', 'Request received via email', 4, '{"requires_signature": false, "processing_days": 2}'),
    ('receipt_form', 'personal', 'Personal Visit', 'Request submitted in person', 5, '{"requires_signature": true, "processing_days": 1}'),
    ('receipt_form', 'mobile_app', 'Mobile Application', 'Request submitted through mobile app', 6, '{"requires_signature": false, "processing_days": 1}');
  `);
  ...
  await db.exec(`
    ...
    ('admin', 'admin@city.gov', '${adminPasswordHash}', 'System Administrator', 'admin', 'IT', 'System Administrator'),
    ('water_manager', 'water.manager@city.gov', '${adminPasswordHash}', 'Water Department Manager', 'executor', 'Water Supply Department', 'Department Manager'),
    ('roads_manager', 'roads.manager@city.gov', '${adminPasswordHash}', 'Roads Department Manager', 'executor', 'Road Maintenance Department', 'Department Manager'),
    ('clerk', 'clerk@city.gov', '${adminPasswordHash}', 'Office Clerk', 'user', 'Administrative Office', 'Clerk');
  `);
}
...
async function down(db) {
  ...
  await db.exec('DELETE FROM nomenclature WHERE type IN ("priority", "request_type", "topic", "receipt_form", "executor")');
}
